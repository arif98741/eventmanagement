...
class Login {
    ...
    */
    ...
    public function addUser($data)
    {
        ...
        $stmt = $this->dbObj->link->query("select * from user where username='admin'");
        if ($stmt) {
            ...
            $checkQ = $this->dbObj->link->query("select * from user where username = '$username' or email = '$email'");
            if ($checkQ) {
                if($checkQ->num_rows > 0){
                    $msg = "<script>alert('Stuff already Exist');</script>";
                }else{
                     $stmt1 = $this->dbObj->link->query("insert into user(name,username,password,email,company_name,address,logo) values('$name','$username','$password','$email','$company_name','$address','$logo')");
                     if ($stmt1) {
